...
function AddCategory(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    setBtnTxt("Uploading...");
    setUploading(true);
    if (category.length > 0 && file !== null) {
      toast.info("Uploading Category. Please Wait...");
      let _f = "";
      try {
        _f = await uploadToFirebaseStorage(`categories/`, file);
      } catch (e) {
      ...
      console.log({ title: category, enabled, image: _f });
      postData("/admin/category", { title: category, enabled, image: _f })
        .then((res) => {
          console.log(res);
          if (!res.error) {
