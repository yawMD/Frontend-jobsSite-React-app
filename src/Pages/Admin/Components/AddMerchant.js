...
function AddMerchant(props) {
  ...
  const [btnTxt, setBtnTxt] = useState("Add New Merchant");

  useEffect(() => {
    getData("/admin/categories")
      .then((e) => {
        console.log(e);
        setCategoryList(e.categories === undefined ? [] : e.categories);
      })
  ...
  const handleCheck = k => {
      let _c = []
      if(category.includes(categoryList[k].title)) {
          _c = category.flatMap(c => c === categoryList[k].title ? [] : [c])
      } else {
          _c = [...category, categoryList[k].title]
      }
      setCategory(_c)
  }
  const handleSubmit = (e) => {
    e.preventDefault();
    let _data = {category, name, email, business, phone, password, province, status: 'active' };
    console.log(_data);
    postData("/admin/user/merchant", _data)
  ...
  return (
    <div className="h-screen fixed justify-center content-center flex top-0 left-0 z-50 w-full bg-white bg-opacity-10">
      <form className="my-auto w-1/2" onSubmit={handleSubmit}>
        ...
        <hr className="my-2" />
        <h2 className="text-3xl font-bold">Add New Freelancer</h2>
        {/* <hr className="my-2" /> */}
        <div className="my-2 p-6 col-s pan-2 md:col-spa n-1">
          ...
          <div className="mt-3">
            <input
              className="w-full shadow rounded-md border-2 border-gray-100 hover:border-gray-200 p-3 font-semibold"
              placeholder="Freelancer's Business Name"
              value={business}
              required
          ...
          <div className="mt-3">
            ...
            {categoryList.map((c, k) => (
              <label key={k} className="mb-2 mr-2 rounded p-2">
                <input
                  className="p-3"
                  type="checkbox"
          ...
          {/* <div className="mt-2">
            <label htmlFor="doc">Freelancer's Profile Picture</label>
          ...
          <div className="mt-3">
            <label htmlFor="doc">Freelancer's Documents</label>
